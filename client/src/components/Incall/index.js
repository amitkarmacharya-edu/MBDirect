...
function InCall({}) {
    ...
    useEffect(() => {
        ...
        meets.remoteCam = remoteCam;
        if(!meets.signalingChannel){
        ...
        console.log(meets.userMedia.localStream);
        if(meets && meets.userMedia && meets.userMedia.haveLocalStream){
            meets.gotLocalStream();
        }
        startConnection()
    }, []);
    function startConnection() {
        if (meets.userId && meets.roomId && meets.businessId && meets.userMedia && meets.userMedia.localStream){
            meets.connectToPeers();
        } else {
            dispatch({
                type: UPDATE_STAGE,
    ...
    async function handleCallEnd() {
        dispatch({type: LOADING});
        await API.leaveRoom({userId: state.userId, roomId: state.roomId});
        meets.closeMeeting();
        console.log(meets);
        ...
        <div className="row bg-dark h-100">
            <div className="col-12 p-2">
                ...
                <div className="col-12 controls">
                    ...
                    {/* end call */}
                    <button type="button" className="btn btn-secondary bg-dark text-danger px-4 w-25" onClick={handleCallEnd}>
